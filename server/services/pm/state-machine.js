...
//
// Espelha 1:1 os CHECK constraints definidos nas migrations 028+. Manter
...
// (Port do IMPGEO com poda do TerraControl/PIX.)
// ═══════════════════════════════════════════════════════════════════════════

'use strict';

// ─── Projeto (status) ─────────────────────────────────────────────────────────
// Espelha CHECK em projects.status. Em PORTUGUÊS (migration 028).
...
// ─── Projeto (source) ─────────────────────────────────────────────────────────
// Espelha CHECK em projects.source (sem 'terracontrol_pix' — poda).
const PROJECT_SOURCES = Object.freeze(['manual', 'imported']);

// ─── Clients (source) ─────────────────────────────────────────────────────────
// Espelha CHECK em clients.source (sem 'terracontrol' — poda).
const CLIENT_SOURCES = Object.freeze(['manual', 'imported']);

// ─── project_events ───────────────────────────────────────────────────────────
// Espelha CHECK em project_events.actor_type (sem 'abacatepay' — poda).
const PROJECT_EVENT_ACTOR_TYPES = Object.freeze(['user', 'system', 'cron']);

// Lista de tipos de evento de projeto (sem 'project_created_from_pix' — poda).
...
// ─── Task (status) ────────────────────────────────────────────────────────────
// CHECK em project_tasks.status (migration 030). 10 estados.
...
// Matriz de transições válidas. Estrutura: { [from]: [to, ...] }.
const ALLOWED_TRANSITIONS = Object.freeze({
  ...
  // 'completed' deixou de ser terminal: pode ser REABERTA (desconcluída),
  // voltando para 'available' — o responsável precisa dar play de novo.
  completed:          ['available'],
  canceled:           [],   // terminal
});
