-- =============================================================================
-- Migration 021: role_default_permissions (Fase 2.2 do alya)
-- =============================================================================
--
-- Espelha a migration 043 do impgeo, adaptada ao schema do alya (tabela
-- `modules`, coluna `key`).
--
-- Move o mapa de defaults por role do hardcoded (server/permissions/defaults.js
-- — a ser criado na Fase 2.4) para uma tabela persistente que superadmins
-- podem editar via UI (Fase 2.7 — "Padrões de Função").
--
-- Cada (role, module_key) representa: "essa role recebe esse módulo, com
-- esse access_level, ao ser criada ou resetada." Ausência = sem acesso.
--
-- O mapa hardcoded permanecerá em defaults.js como FALLBACK_DEFAULTS — usado
-- quando o banco não tem nenhum registro pra uma role (edge case de deletar
-- tudo) e como referência do "padrão original" no botão "Restaurar padrão".
--
-- Seed inicial reflete EXATAMENTE a tabela da migration 020:
--
--   ┌─────────────┬───────┬────────┬──────────┬───────────────┬──────────┐
...
--   └─────────────┴───────┴────────┴──────────┴───────────────┴──────────┘
--   ¹ admin: só 'admin' (UserManagement); activeSessions/anomalies/
--     securityAlerts permanecem exclusivos do superadmin (sem registro).
--   ² guest/gestao: só faq + documentacao (sem roadmap).
--
-- Reversão: 021 - ROLE-DEFAULT-PERMISSIONS-rollback.sql.
-- =============================================================================
...
INSERT INTO role_default_permissions (role, module_key, access_level)
SELECT 'guest', key, 'view'
  FROM modules
 WHERE is_active = TRUE
   AND (
        (subsystem_key = 'gestao' AND key IN ('faq', 'documentacao'))
...
BEGIN
  SELECT COUNT(*) INTO v_active_mods FROM modules WHERE is_active = TRUE;
  ...
  RAISE NOTICE '✓ Migration 021: role_defaults populados — super=% admin=% mgr=% user=% guest=%',
    v_count_super, v_count_admin, v_count_mgr, v_count_user, v_count_guest;
END $$;
