// =============================================================================
// PermissionsMatrix — UI granular de permissões (Fase 2.6 do alya)
// =============================================================================
//
// Port literal do impgeo. Substitui grids binários (enabled/disabled) por
// uma matriz agrupada por subsistema com 3 níveis por módulo:
//   ○ Sem acesso  ○ Ver  ○ Editar
//
// Header de cada subsistema permite "marcar tudo" em qualquer dos 3 níveis
// (bulk apply client-side; payload final é UM PUT em
// /api/admin/users/:id/permissions). Botão "Resetar para defaults da role"
// chama POST /api/admin/users/:id/permissions/reset-to-defaults.
//
// Componente é controlado pelo pai via prop `permissions` + onChange.
// Carregamento/salvamento ficam fora — facilita testes e reuso. Hook
// utilitário em src/hooks/useUserPermissions.ts encapsula esse fluxo.
// =============================================================================
...
interface PermissionsMatrixProps {
  /** Matriz atual (derivada de GET /permissions + catálogo de módulos). */
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 bg-gray-50 dark:!bg-[#1f2937] rounded-lg p-3 border border-gray-200 dark:border-gray-700">
        <div className="text-sm text-gray-700 dark:text-gray-200">
          ...
          <span className="text-gray-500">Defina o nível por submódulo ou aplique o nível ao subsistema inteiro pelo botão "Aplicar a todos" do cabeçalho.</span>
        </div>
        <button
          type="button"
      ...
      {grouped.map(({ subsystem, items }) => {
        ...
        return (
          ...
          >
            <legend className="sr-only">Permissões do subsistema {subsystem.name}</legend>
