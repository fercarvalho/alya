...
const CATEGORIA_CONFIG = {
  ...
  sugestao: { label: 'Sugestão', icon: Lightbulb, bg: 'bg-amber-100', text: 'text-amber-700' },
...
const FeedbackDetailModal: React.FC<FeedbackDetailModalProps> = ({ feedback, onClose, onAtualizar }) => {
  ...
  const handleAcao = async () => {
    if (!mensagem.trim()) {
      setErro('Escreva uma mensagem antes de confirmar.');
      return;
    }
    ...
    try {
      const endpoint = acao === 'aceitar'
        ? `${API_BASE_URL}/admin/feedbacks/${feedback.id}/aceitar`
        : `${API_BASE_URL}/admin/feedbacks/${feedback.id}/responder`;

      const body = acao === 'aceitar'
        ? { mensagem: mensagem.trim(), criarRoadmap: true }
  ...
  return (
    <>
      ...
      >
        <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl p-6 w-full max-w-xl max-h-[calc(100vh-220px)] overflow-y-auto shadow-2xl border border-gray-200/50">
          {/* Header */}
          ...
          <div className="space-y-5">
            ...
            {feedback.linkVideo && (
              <div>
                ...
                <a
                  ...
                  className="flex items-center gap-2 text-sm text-amber-600 hover:text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-xl px-4 py-3 transition-colors"
                >
            ...
            {/* Painel de ação — superadmin + pendente */}
            {podeAgir && (
              <div className="border-2 border-dashed border-amber-200 rounded-xl p-4 bg-amber-50/50">
                {!acao ? (
                  <div>
                    <p className="text-xs font-semibold text-amber-700 uppercase tracking-wider mb-3">Ações disponíveis</p>
                    <div className="flex gap-3">
                      ...
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-xl text-sm font-semibold shadow-sm transition-all"
                      >
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-semibold text-amber-700 uppercase tracking-wider">
                        {acao === 'aceitar' ? 'Aceitar e criar tarefa no Roadmap' : 'Responder usuário'}
                      </p>
                      <button
                    ...
                    {acao === 'aceitar' && (
                      <div className="bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-lg text-sm flex items-start gap-2">
                        ...
                        <span>Este feedback será aceito, uma tarefa será criada no Roadmap e o usuário receberá um email de confirmação com sua mensagem.</span>
                      </div>
                    )}

                    {acao === 'responder' && (
                      <div className="bg-blue-50 border border-blue-200 text-blue-800 px-4 py-3 rounded-lg text-sm flex items-start gap-2">
                        <MessageCircle className="w-4 h-4 shrink-0 mt-0.5 text-blue-600" />
                        <span>O usuário receberá um email com sua resposta.</span>
                      </div>
                    )}
                    ...
                    <button
                      onClick={handleAcao}
                      disabled={isSubmitting}
                      className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold shadow-sm transition-all disabled:opacity-60 disabled:cursor-not-allowed ${
                        acao === 'aceitar'
            ...
            {/* Aviso para admin sem permissão */}
