...
const VersaoNovaModal: React.FC<Props> = ({ versoes, onConfirm, onClose }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-amber-500 to-orange-500 rounded-t-xl px-6 py-5 relative overflow-hidden">
          ...
          {total > 1 && (
            <div className="flex items-center justify-center gap-2 mt-4">
              {pendentes.map((_, i) => (
                <span
                  key={i}
                  className={`h-1.5 rounded-full transition-all ${i === index ? 'w-6 bg-amber-500' : 'w-1.5 bg-gray-300'}`}
                />
              ))}
            </div>
          )}
        ...
        <div className="px-6 pb-5 flex items-center justify-between gap-3">
          ...
          <button onClick={handleEntendido}
            className="px-5 py-2 text-sm font-medium bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white rounded-lg transition-colors">
            Entendido!
          </button>
        </div>
      </div>
