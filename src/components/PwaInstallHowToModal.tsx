...
function getInstructions(strategy: InstallStrategy): InstructionConfig | null {
  switch (strategy) {
    case 'ios-safari':
      return {
        title: 'Instalar Alya no iPhone / iPad',
        steps: [
          {
          ...
          },
          {
            ...
            text: <>Confirme em <strong>"Adicionar"</strong>. O ícone do Alya aparece na sua tela.</>,
          },
        ],
    ...
    case 'macos-safari':
      return {
        title: 'Instalar Alya no Mac (Safari)',
        steps: [
          {
            icon: <span className="text-base font-semibold">📂</span>,
    ...
    case 'android-firefox':
      return {
        title: 'Instalar Alya no Android (Firefox)',
        steps: [
          {
    ...
    case 'unsupported':
      return {
        ...
        steps: [
          ...
          {
            icon: <span className="text-base font-semibold">✓</span>,
            text: <>Abra o Alya no <strong>Chrome</strong>, <strong>Edge</strong> ou <strong>Safari</strong> pra ver o botão de instalar.</>,
          },
        ],
      }
    default:
        ...
        <ol className="space-y-3">
          {config.steps.map((step, idx) => (
            <li key={idx} className="flex items-start gap-3">
              <span className="flex-shrink-0 w-8 h-8 rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 flex items-center justify-center text-sm font-semibold">
                {idx + 1}
              </span>
              <div className="flex-1 pt-1.5 text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
                <span className="inline-flex items-center gap-2">
                  <span className="text-amber-600 dark:text-amber-400">{step.icon}</span>
                  <span>{step.text}</span>
                </span>
              </div>
            </li>
