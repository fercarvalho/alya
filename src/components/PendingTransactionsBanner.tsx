...
/**
 * Banner amber exibido no Dashboard, DRE e Transações quando há transações
 * pendentes de confirmação. Avisa que totais podem estar incompletos.
...
const PendingTransactionsBanner: React.FC = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('alya:transactions-changed', onChanged)
    return () => {
      clearInterval(t)
      window.removeEventListener('alya:transactions-changed', onChanged)
    }
  }, [refresh])
