...
const NotificationBell: React.FC = () => {
  ...
  const handleClickNotification = async (n: Notification) => {
    ...
    if (n.notificationType === 'transaction_confirm_needed' && n.relatedEntityId) {
      setResolving({ transactionId: n.relatedEntityId, description: n.message || '' })
      setOpen(false)
    }
  }
            ...
            <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/30 dark:to-orange-900/30">
              ...
              {items.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {unreadCount > 0 && (
                    <button onClick={markAllRead} className="flex items-center gap-1 text-[11px] px-2 py-1 bg-amber-100 hover:bg-amber-200 dark:bg-amber-900/40 dark:hover:bg-amber-900/60 text-amber-800 dark:text-amber-200 rounded font-semibold">
                ...
                <ul className="divide-y divide-gray-100 dark:divide-gray-700">
                  {items.map((n) => (
                    <li
                      ...
                      className={`group relative p-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${n.isRead ? 'opacity-70' : ''}`}
                    >
                      <div className="flex items-start gap-2">
                        {!n.isRead && <span className="mt-1.5 w-2 h-2 bg-amber-500 rounded-full flex-shrink-0" />}
                      ...
                      <div className="absolute top-2 right-2 hidden group-hover:flex gap-1 bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 p-0.5" onClick={stop}>
                        ...
                        <button onClick={() => clearOne(n.id)} title="Limpar (esconde do sininho)" className="p-1.5 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded">
                          <EyeOff className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => deleteOne(n.id)} title="Excluir definitivamente" className="p-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded">
